...
export function PlanPage({ sales, product, prediction, predictionError, loading, finalQuantity, setFinalQuantity, onSaved }) {
  ...
  const justSaved = savedQuantity !== null && savedQuantity === finalQuantity;
  const finalChange = finalQuantity - baseline;
  const finalSavings = savings(baseline, finalQuantity, product?.unit_cost);
  const finalRisk = wasteRisk(finalQuantity, predicted);
  const handleSaveClick = () => {
    onSaved({ baseline, predicted, adjusted, finalQuantity, model: prediction?.model || 'fallback' });
    setSavedQuantity(finalQuantity);
  };
  ...
  return <><section className="page-heading plan-heading"><div><span className="eyebrow">시연용 예시 데이터</span><h1>내일 생산계획</h1><p>맛있는 하루가, 더 오래 이어지도록</p></div><RiskBadge level={risk.level}/></section><section className="plan-grid">
    ...
    {justSaved ? <div className="save-confirm">
      <Icon name="check" size={26}/>
      <div>
        <strong>오늘 생산계획이 확정되었습니다</strong>
        <p>{productName} {finalQuantity}개 · 기존 계획({baseline}개) 대비 {finalChange === 0 ? '동일' : finalChange < 0 ? `${-finalChange}개 감소` : `${finalChange}개 증가`}{finalSavings.amount != null && finalSavings.amount > 0 ? ` · 약 ${formatWon(finalSavings.amount)} 절감 예상` : ''}</p>
      </div>
      <RiskBadge level={finalRisk.level}/>
      <button className="text-button" onClick={() => setSavedQuantity(null)}>다시 수정하기</button>
    </div> : <button className="save-plan" onClick={handleSaveClick}><Icon name="save" size={25}/><span>계획 저장</span><b>→</b></button>}
